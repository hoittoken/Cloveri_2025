...
- [Пример SQL запросов](#примеры-sql-запросов)
- [Фреймворк для реализации](#фреймворк-для-реализации)
- [Костыльный пример исполнения](#костыльный-пример-исполнения)
...
* Переносимая. SQLAlchemy, при должном подходе, позволяет писать код на Python, совместимый с несколькими back-end СУБД. Несмотря на стандартизацию языка SQL, между базами данных имеются различия в его реализации, абстрагироваться от которых и помогает SQLAlchemy.
        ...
        Сильно любимый мной Pandas при работе с SQL настоятельно рекомендует использовать SQLAlchemy, о чём, в случае не выполнения этой рекомендации будет выкидывать ошибку:
